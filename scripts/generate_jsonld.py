...
class AIClient:
    ...
    def _add_ai_disclosure(self, data: Dict) -> Dict:
        ...
        if keywords is None:
            ...
            new_keywords = [k for k in keywords if k not in ("ChatGPT", AI_GENERATED_KEYWORD)]
            new_keywords.append(AI_GENERATED_KEYWORD)
        elif isinstance(keywords, str):
            new_keywords = (
                [keywords, AI_GENERATED_KEYWORD]
                if keywords not in (AI_GENERATED_KEYWORD, "ChatGPT")
                else [AI_GENERATED_KEYWORD]
            )
        else:
            new_keywords = [AI_GENERATED_KEYWORD]
        ...
        if isinstance(props, list):
            filtered = [
                ...
                if not (
                    isinstance(p, dict)
                    and p.get("propertyID") in ("aiGeneratedMetadata", "aiSystemUsed")
                )
            ]
            if filtered:
                new_props = filtered if len(filtered) != 1 else filtered[0]
        elif isinstance(props, dict):
            if props.get("propertyID") not in ("aiGeneratedMetadata", "aiSystemUsed"):
                new_props = props

        # strip legacy AI comment text
        ...
        if isinstance(comment, list):
            kept_c = [
                c
                for c in comment
                if not (isinstance(c, str) and "generated with the use of AI" in c)
            ]
            if kept_c:
                new_comment = kept_c if len(kept_c) != 1 else kept_c[0]
        elif isinstance(comment, str):
            if "generated with the use of AI" not in comment:
                new_comment = comment
        ...
        if new_props is not None:
            out["additionalProperty"] = new_props
        if new_comment is not None:
            # place comment after @type when present
            if "@type" in out:
                rebuilt = {}
                for k, v in out.items():
                    rebuilt[k] = v
                    if k == "@type":
                        rebuilt["comment"] = new_comment
                out = rebuilt
            else:
